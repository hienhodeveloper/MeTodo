...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if (children.count == 0) {
            ...
            let colorScheme = colorSchemes[(children[0] as! ListViewController).taskList.colorSchemeId]
    ...
    private func addLists() {
        ...
        let newTaskListButtonXPos = kBuffer + (kBuffer * CGFloat(children.count)/2) + (CGFloat(children.count) * MINIMIZED_LIST_WIDTH)
        ...
        listScrollView.contentSize.width = view.frame.width*CGFloat(children.count + 1)
    }
    ...
    func changeListSize(gestureDirection: UISwipeGestureRecognizer.Direction, editing: Bool) {
        let lvc = self.children[listIndex] as! ListViewController
        ...
        switch (gestureDirection) {
            case .up:
                ...
                listScrollView.bringSubviewToFront(lvc.view)
                lvc.expand()
    ...
    private func presentCustomListView(newList: Bool) {
        ...
        if (listIndex < children.count) {
            let lvc = children[listIndex] as! ListViewController
